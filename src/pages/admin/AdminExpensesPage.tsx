    ...
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold font-serif text-primary flex items-center gap-2">
            <Receipt className="w-6 h-6" /> Expense Tracker
          </h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-card border border-border p-4 rounded-xl shadow-xs">
          ...
          <p className="text-2xl font-bold text-foreground">{formatCurrency(stats.total || 0)}</p>
        </div>
        <div className="bg-card border border-border p-4 rounded-xl shadow-xs">
          <p className="text-sm font-medium text-muted-foreground mb-1">Approved & Paid</p>
          <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{formatCurrency(stats.approved || 0)}</p>
        </div>
        <div className="bg-card border border-border p-4 rounded-xl shadow-xs relative overflow-hidden">
          <p className="text-sm font-medium text-muted-foreground mb-1">Pending Approval</p>
          <p className="text-2xl font-bold text-amber-600 dark:text-amber-400">{formatCurrency(stats.pending || 0)}</p>
        ...
        <div className="bg-card border border-border p-4 rounded-xl shadow-xs">
          <p className="text-sm font-medium text-muted-foreground mb-1 flex items-center gap-1">
            <FolderOpen className="w-3.5 h-3.5" /> Case-Linked Costs
          </p>
          <p className="text-2xl font-bold text-foreground">{formatCurrency(stats.caseLinked || 0)}</p>
        </div>
      </div>
                  ...
                  <tr key={exp._id} className="hover:bg-muted/30 transition-colors group">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-1 items-start">
                        <Badge variant="secondary" className="text-[10px] font-normal">{CATEGORIES[exp.category] || exp.category}</Badge>
                        {exp.caseId && (
                          <Badge variant="outline" className="text-[10px] gap-1 font-normal text-blue-600 border-blue-200 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-400">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        {exp.status === "pending" && (
                          <>
                            <button onClick={() => handleStatusUpdate(exp._id, "approved")} className="p-1.5 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-md" title="Approve">
                              <CheckCircle2 className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleStatusUpdate(exp._id, "rejected")} className="p-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md" title="Reject">
                              <XCircle className="w-4 h-4" />
                            </button>
                          </>
                        )}
                        <button onClick={() => handleDelete(exp._id)} className="p-1.5 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-md" title="Delete Record">
