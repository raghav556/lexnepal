...
export default function ClientBookingPage() {
  ...
  const { data: availableSlots = [] } = useAvailableSlots(dateStr);
  const { bookConsultation } = useAppointmentCommands();
  const { data: allAppointments = [] } = useAppointments(
    assignedLawyerId ? { assignedLawyerId } : {},
  );
  const appointments = useMemo(() => {
    if (!clientId && !clientRecord?.email) return [] as any[];
    return (allAppointments as any[]).filter((a) =>
      (clientId && a.clientId === clientId) ||
      (clientRecord?.email && a.clientEmail === clientRecord.email),
    );
  }, [allAppointments, clientId, clientRecord?.email]);
            ...
            {upcomingAppointments.length === 0 ? (
              ...
              <div className="space-y-3">
                {upcomingAppointments.map((apt: any) => (
                  <div key={apt._id || apt.id} className="p-3 rounded-lg border border-border bg-muted/30 text-sm">
