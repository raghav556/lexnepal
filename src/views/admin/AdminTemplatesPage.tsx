...
import { useDocumentTemplateCommands, useDocumentTemplates } from "@/client/queries/templates";
...
export default function AdminTemplatesPage() {
  ...
  const { createTemplate, updateTemplate, deleteTemplate, seedTemplates } =
    useDocumentTemplateCommands();
  ...
  return (
    <PortalPageShell
      ...
      description="Manage standard legal templates for document assembly."
      icon={FileText}
      actions={
        <div className="flex gap-2">
          <DashboardButton variant="outline" onClick={handleSeed} disabled={isSeeding}>
            {isSeeding ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : null}
            Seed Default Templates
          </DashboardButton>
          <DashboardButton onClick={() => handleOpenModal()} className="shrink-0 gap-2">
            <Plus className="w-4 h-4" /> New Template
          </DashboardButton>
        </div>
      }
    >
      <DashboardSection title="Template library" icon={FileText}>
        {templates && templates.length === 0 ? (
          <EmptyState
            title="No templates found"
            description="Create a template or seed the default library to get started."
            icon={FileText}
          />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {(templates || []).map((template: any) => (
              ...
              >
                <div className="h-full flex flex-col rounded-xl border border-dashboard-border bg-dashboard-panel p-5 hover:border-dashboard-primary/50 transition-colors">
                  <div className="flex justify-between items-start mb-2">
                    <div className="p-2 bg-dashboard-primary-soft text-dashboard-primary rounded-lg">
                  ...
                  <div className="mt-auto">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {template.variables.map((v: string) => (
                        <span
                          key={v}
                          className="bg-dashboard-neutral-soft text-foreground text-[10px] px-1.5 py-0.5 rounded border border-dashboard-border"
                        >
                          {v}
                        </span>
                      ))}
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[900px] h-[95vh] sm:h-auto max-h-[95vh] flex flex-col p-4">
          ...
          <div className="space-y-4 py-2 flex-1 overflow-y-auto pr-2">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground">Insert Variables</label>
          ...
          <DialogFooter>
            ...
            <DashboardButton onClick={handleSave} disabled={isSaving}>
