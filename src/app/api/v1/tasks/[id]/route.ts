...
export const GET = withApiHandler("/api/v1/tasks/:id", async ({ request }) => {
  ...
  return jsonResponse({
    data: await getWorkManagementService().getTask(principal, idFrom(request)),
  });
});
...
export const PATCH = withApiHandler("/api/v1/tasks/:id", async ({ request, requestId }) => {
  ...
  return jsonResponse({
    data: await getWorkManagementService().updateTask(
      principal,
      idFrom(request),
      input,
...
export const DELETE = withApiHandler("/api/v1/tasks/:id", async ({ request, requestId }) => {
  const principal = await requireSession(request);
  return jsonResponse({
    data: await getWorkManagementService().deleteTask(
      principal,
      idFrom(request),
      buildAuditContext(request, requestId, principal),
    ),
