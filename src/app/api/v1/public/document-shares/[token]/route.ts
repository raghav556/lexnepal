...
export const POST = withApiHandler("/api/v1/public/document-shares/:token", async ({ request }) => {
  const token = tokenFrom(request);
  const body = await request.json().catch(() => ({}));
  const input = publicDocumentShareSchema.parse({ token, ...(body as object) });
  return jsonResponse({
    data: await getDocumentService().getPublicShare(input.token, input.password),
  });
});
