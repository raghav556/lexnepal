...
export const POST = withApiHandler("/api/v1/envelopes/:id/expire", async ({ request }) => {
  const principal = await requireSession(request);
  return jsonResponse({ data: await getEnvelopeService().expire(principal, idFrom(request)) });
});
