...
function NavSkeleton({ count = 7 }: { count?: number }) {
  return (
    <div className="inline-flex items-center gap-1 p-1 rounded-full border border-border/50 bg-muted/20" aria-busy="true" aria-label="Loading navigation">
...
export function PublicLayoutShell({
  ...
  const { data: headerNav } = useQuery({
    queryKey: queryKeys.cms.collection("public", "navigation", { location: "header" }),
    queryFn: ({ signal }) =>
      apiClient.request<PublicNavEntry[]>("/api/v1/public/cms/navigation", {
        query: { location: "header" },
        signal,
      }),
    initialData: initialHeaderNav,
    staleTime: 60_000,
    retry: 2,
  });
      ...
      <header className={cn(
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-16 gap-3 sm:gap-4 min-w-0">
            <Link href="/" className="flex items-center gap-2 sm:gap-2.5 shrink-0 min-w-0 max-w-[10.5rem] sm:max-w-[12rem]">
              ...
              <div className="flex flex-col min-w-0 leading-tight">
                <span className="font-serif text-base sm:text-xl xl:text-2xl font-bold text-primary tracking-tight truncate">Srimar Law</span>
                <span className="hidden xl:block text-[9px] sm:text-[10px] font-medium text-muted-foreground uppercase tracking-wider mt-0.5 truncate">Attorneys at Law</span>
              </div>
            </Link>
            ...
            <div className="hidden md:flex items-center gap-2 lg:gap-3 shrink-0 ml-auto xl:ml-0">
              <PublicHeaderAuth />
              <Button asChild size="sm" className="bg-accent text-accent-foreground hover:bg-accent/90 shadow-sm font-medium shrink-0 text-xs xl:text-sm px-3 xl:px-4">
            ...
            <button type="button" className="xl:hidden p-2 shrink-0 ml-auto md:ml-0" onClick={() => setMobileOpen((v) => !v)} aria-label={mobileOpen ? "Close menu" : "Open menu"}>
              {mobileOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
            </button>
          </div>
        </div>

        <div className={cn("xl:hidden border-t border-border bg-background overflow-hidden transition-all duration-300 ease-in-out", mobileOpen ? "max-h-[min(70vh,32rem)] opacity-100 overflow-y-auto" : "max-h-0 opacity-0 border-t-0")}>
