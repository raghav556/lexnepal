...
export const POST = withApiHandler("/api/v1/documents/:id/restore", async ({ request }) => {
  const principal = await requireSession(request);
  return jsonResponse({ data: await getDocumentService().restore(principal, idFrom(request)) });
});
