...
export const users = mysqlTable(
  "users",
  {
    ...
    lastLoginAt: utcDateTime("last_login_at", { withTimezone: true }).default(sql`NULL`),
    invitedAt: utcDateTime("invited_at", { withTimezone: true }).default(sql`NULL`),
    invitedBy: uuidColumn("invited_by").references((): AnyMySqlColumn => users.id, {
      onDelete: "set null",
    }),
    inviteExpiresAt: utcDateTime("invite_expires_at", { withTimezone: true }).default(sql`NULL`),
    deactivatedAt: utcDateTime("deactivated_at", { withTimezone: true }).default(sql`NULL`),
...
export const clients = mysqlTable(
  "clients",
  {
    ...
    kycConsentAt: utcDateTime("kyc_consent_at", { withTimezone: true }).default(sql`NULL`),
    kycConsentVersion: stringColumn("kyc_consent_version"),
    kycRejectionReason: stringColumn("kyc_rejection_reason"),
    kycSubmittedAt: utcDateTime("kyc_submitted_at", { withTimezone: true }).default(sql`NULL`),
    kycReviewedAt: utcDateTime("kyc_reviewed_at", { withTimezone: true }).default(sql`NULL`),
...
export const documents = mysqlTable(
  "documents",
  {
    ...
    lockedAt: utcDateTime("locked_at", { withTimezone: true }).default(sql`NULL`),
    physicalLocation: stringColumn("physical_location"),
    expiresAt: utcDateTime("expires_at", { withTimezone: true }).default(sql`NULL`),
    ...
    legalHoldSetAt: utcDateTime("legal_hold_set_at", { withTimezone: true }).default(sql`NULL`),
    legalHoldSetBy: uuidColumn("legal_hold_set_by").references(() => users.id, {
      onDelete: "set null",
    }),
    retentionUntil: utcDateTime("retention_until", { withTimezone: true }).default(sql`NULL`),
    uploadStatus: uploadStatusEnum("upload_status").default("quarantined").notNull(),
    scanProvider: stringColumn("scan_provider"),
    scanCompletedAt: utcDateTime("scan_completed_at", { withTimezone: true }).default(sql`NULL`),
    ...
    signedAt: utcDateTime("signed_at", { withTimezone: true }).default(sql`NULL`),
    ...
    signConsentAt: utcDateTime("sign_consent_at", { withTimezone: true }).default(sql`NULL`),
    viewedAt: utcDateTime("viewed_at", { withTimezone: true }).default(sql`NULL`),
...
export const durableJobs = mysqlTable(
  "durable_jobs",
  {
    ...
    lockedAt: utcDateTime("locked_at", { withTimezone: true }).default(sql`NULL`),
    lockedBy: stringColumn("locked_by"),
    leaseExpiresAt: utcDateTime("lease_expires_at", { withTimezone: true }).default(sql`NULL`),
    lastError: longtext("last_error"),
    result: json("result"),
    ...
    completedAt: utcDateTime("completed_at", { withTimezone: true }).default(sql`NULL`),
    deadLetteredAt: utcDateTime("dead_lettered_at", { withTimezone: true }).default(sql`NULL`),
    manualRetryCount: integer("manual_retry_count").default(0).notNull(),
    lastManualRetryAt: utcDateTime("last_manual_retry_at", { withTimezone: true }).default(
      sql`NULL`,
    ),
...
export const durableSchedules = mysqlTable(
  "durable_schedules",
  {
    ...
    timeoutSeconds: integer("timeout_seconds").default(300).notNull(),
    isActive: boolean("is_active").default(true).notNull(),
    lastEnqueuedAt: utcDateTime("last_enqueued_at", { withTimezone: true }).default(sql`NULL`),
    ...lifecycleColumns(),
  },
  (table) => [
...
export const storageMigrationItems = mysqlTable(
  "storage_migration_items",
  {
    ...
    attempts: integer("attempts").default(0).notNull(),
    lastError: longtext("last_error"),
    verifiedAt: utcDateTime("verified_at", { withTimezone: true }).default(sql`NULL`),
    ...lifecycleColumns(),
  },
  (table) => [
...
export const documentShares = mysqlTable(
  "document_shares",
  {
    ...
    lockedUntil: utcDateTime("locked_until", { withTimezone: true }).default(sql`NULL`),
    lastAccessAt: utcDateTime("last_access_at", { withTimezone: true }).default(sql`NULL`),
    revokedAt: utcDateTime("revoked_at", { withTimezone: true }).default(sql`NULL`),
...
export const signatureEnvelopes = mysqlTable(
  "signature_envelopes",
  {
    ...
    createdBy: uuidColumn("created_by")
      ...
      .references(() => users.id, { onDelete: "restrict" }),
    expiresAt: utcDateTime("expires_at", { withTimezone: true }).default(sql`NULL`),
    voidedAt: utcDateTime("voided_at", { withTimezone: true }).default(sql`NULL`),
    voidReason: stringColumn("void_reason"),
    completedAt: utcDateTime("completed_at", { withTimezone: true }).default(sql`NULL`),
    lastRemindedAt: utcDateTime("last_reminded_at", { withTimezone: true }).default(sql`NULL`),
    ...lifecycleColumns(),
  },
  (table) => [
...
export const signatureRecipients = mysqlTable(
  "signature_recipients",
  {
    ...
    declinedAt: utcDateTime("declined_at", { withTimezone: true }).default(sql`NULL`),
    declineReason: stringColumn("decline_reason"),
    signedAt: utcDateTime("signed_at", { withTimezone: true }).default(sql`NULL`),
    remindedAt: utcDateTime("reminded_at", { withTimezone: true }).default(sql`NULL`),
    ...lifecycleColumns(),
  },
  (table) => [
...
export const tasks = mysqlTable(
  "tasks",
  {
    ...
    dueDate: utcDateTime("due_date", { withTimezone: true }).default(sql`NULL`),
    dueDateBs: stringColumn("due_date_bs"),
    isRecurring: boolean("is_recurring").default(false).notNull(),
    recurrenceRule: recurrenceEnum("recurrence_rule"),
    reminderAt: utcDateTime("reminder_at", { withTimezone: true }).default(sql`NULL`),
    completedAt: utcDateTime("completed_at", { withTimezone: true }).default(sql`NULL`),
    archivedAt: utcDateTime("archived_at", { withTimezone: true }).default(sql`NULL`),
    ...
    lastDueReminderAt: utcDateTime("last_due_reminder_at", { withTimezone: true }).default(
      sql`NULL`,
    ),
    ...lifecycleColumns(),
  },
...
export const dmThreads = mysqlTable(
  "dm_threads",
  {
    ...
    userHighId: uuidColumn("user_high_id")
      .notNull()
      .references(() => users.id, { onDelete: "restrict" }),
    lastMessageAt: utcDateTime("last_message_at", { withTimezone: true }).default(sql`NULL`),
    ...lifecycleColumns(),
  },
  (table) => [
...
export const attendance = mysqlTable(
  "attendance",
  {
    ...
    clockIn: utcDateTime("clock_in", { withTimezone: true }).default(sql`NULL`),
    clockOut: utcDateTime("clock_out", { withTimezone: true }).default(sql`NULL`),
...
export const payrollRuns = mysqlTable(
  "payroll_runs",
  {
    ...
    finalizedAt: utcDateTime("finalized_at", { withTimezone: true }).default(sql`NULL`),
    ...lifecycleColumns(),
  },
  (table) => [
...
export const authUsers = mysqlTable(
  "auth_users",
  {
    ...
    banExpires: utcDateTime("ban_expires", { withTimezone: true }).default(sql`NULL`),
    createdAt: utcDateTime("created_at", { withTimezone: true }).defaultNow().notNull(),
    updatedAt: utcDateTime("updated_at", { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => [
...
export const authAccounts = mysqlTable(
  "auth_accounts",
  {
    ...
    accessTokenExpiresAt: utcDateTime("access_token_expires_at", { withTimezone: true }).default(
      sql`NULL`,
    ),
    refreshTokenExpiresAt: utcDateTime("refresh_token_expires_at", { withTimezone: true }).default(
      sql`NULL`,
    ),
...
export const newsAndAwards = mysqlTable(
  "news_and_awards",
  {
    ...
    submittedAt: utcDateTime("submitted_at", { withTimezone: true }).default(sql`NULL`),
    reviewedBy: uuidColumn("reviewed_by"),
    reviewedAt: utcDateTime("reviewed_at", { withTimezone: true }).default(sql`NULL`),
...
export const blogPosts = mysqlTable(
  "blog_posts",
  {
    ...
    publishDate: utcDateTime("publish_date", { withTimezone: true }).default(sql`NULL`),
    seoTitle: stringColumn("seo_title"),
    seoDescription: longtext("seo_description"),
    displayOrder: integer("display_order").default(0).notNull(),
    isFeatured: boolean("is_featured").default(false).notNull(),
