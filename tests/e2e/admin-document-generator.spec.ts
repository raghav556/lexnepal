...
test("document generator resolves editor variables and clears a case when the client changes", async ({ page }) => {
  ...
  const data = {
    "document-templates": [{
      _id: "template-one", title: "Generator regression", category: "other",
      htmlContent: "<p>Client: {{client.name}}</p><p>Case: {{case.title}} ({{case.number}})</p><p>Lawyer: {{lawyer.name}}</p>",
    ...
    cases: [{ _id: "case-one", clientId: "client-one", title: "Test Matter", caseNumber: "TEST-01", assignedLawyerId: "lawyer-one" }],
  ...
  await page.route("**/api/v1/document-upload-intents", route => route.fulfill({
    status: 201, json: { data: { intentId: "upload-one", upload: { url: "/api/v1/storage/uploads/test", fields: {} } } },
  }));
  await page.route("**/api/v1/storage/uploads/test", route => route.fulfill({ status: 201, json: {} }));
  await page.route("**/api/v1/document-upload-intents/upload-one/complete", route => route.fulfill({ status: 202, json: { data: { status: "queued" } } }));
  ...
  await expect(page.locator("main pre")).toHaveText("Client: Test Client\nCase: Test Matter (TEST-01)\nLawyer: Test Lawyer");
  await expect(page.getByText("Document generated and saved to case files.", { exact: true })).toBeVisible();
});
